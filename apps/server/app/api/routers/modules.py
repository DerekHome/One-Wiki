...
def enable_module(module_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    check_admin(current_user)
    module_registry.enable(module_id)
    module_registry.persist_to_db(db, module_id)
    return ResponseModel(data={"message": f"模块 {module_id} 已启用", "status": "enabled"})

@router.post("/{module_id}/disable")
def disable_module(module_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    check_admin(current_user)
