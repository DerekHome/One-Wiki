<template>
  <div v-loading="loading" class="max-w-5xl mx-auto pb-12">
    <div class="mb-4 flex items-center justify-between">
      ...
      <h2 class="kh-title text-xl">版本差异对比 (Version Diff)</h2>
      <div class="flex items-center gap-2">
        <span class="text-xs text-[var(--kh-text-muted)]">对比版本:</span>
        <el-tag size="small" type="info">v{{ vFrom }}</el-tag>
        <span class="text-xs text-[var(--kh-text-dim)]">vs</span>
        <el-tag size="small" type="primary">v{{ vTo }}</el-tag>
      </div>
    </div>

    <el-card shadow="never" class="rounded-xl mb-4 p-4">
      <h3 class="font-bold text-[var(--kh-text-soft)] mb-2">标题差异</h3>
      <div class="text-sm bg-[var(--kh-surface-soft)] p-3 rounded border border-[var(--kh-border)] font-mono">
    ...
    <el-card shadow="never" class="rounded-xl p-4">
      <h3 class="font-bold text-[var(--kh-text-soft)] mb-2">正文差异 (Unified Diff)</h3>
