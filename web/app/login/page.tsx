...
function LoginContent() {
  ...
  const [error, setError] = useState(searchParams.get("error") ? "登录失败，请检查用户名和密码" : "");
    ...
    <main className="auth">
      <form className="auth-card" action="/auth/login" method="post" onSubmit={submit}>
        ...
        <div className="form-grid">
          <div className="field">
            <label>用户名</label>
            <input name="username" value={username} onChange={(event) => setUsername(event.target.value)} required />
          </div>
          <div className="field">
            <label>&#x5bc6;&#x7801;</label>
        ...
        <p className="helper" style={{ marginTop: 18 }}>默认管理员：系统管理员 / ChangeMe123!</p>
