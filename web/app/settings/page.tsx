...
import { AdminSettings, api, Group, Page, User } from "@/lib/api";
...
export default function SettingsPage() {
  ...
  const [newUser, setNewUser] = useState({ display_name: "", email: "", password: "", role: "reader" });
  ...
  async function createUser(event: FormEvent) {
    ...
    await run(async () => { await api("/admin/users", { method: "POST", body: JSON.stringify({ ...newUser, is_active: true }) }); setNewUser({ display_name: "", email: "", password: "", role: "reader" }); await load(); }, "用户已创建");
  }
  async function updateUser(user: User, changes: Partial<{ role: string; is_active: boolean }>) {
    await run(async () => { await api(`/admin/users/${user.id}`, { method: "PUT", body: JSON.stringify({ email: user.email, display_name: user.name, role: changes.role ?? user.role, is_active: changes.is_active ?? user.is_active }) }); await load(); }, "用户信息已更新");
  }
  ...
  return <Shell><section className="settings-page">
    ...
    <div className="settings-layout"><nav className="settings-nav">{tabs.map((item) => <button key={item.id} className={tab === item.id ? "active" : ""} onClick={() => setTab(item.id)}>{item.label}</button>)}</nav><div className="settings-content">
      {tab === "overview" && settings && <div className="settings-panel"><h2>系统概览</h2><div className="settings-stats"><div><strong>{pages.length}</strong><span>文档</span></div><div><strong>{users.length}</strong><span>用户</span></div><div><strong>{groups.length}</strong><span>群组</span></div><div><strong>{settings.current_database_driver}</strong><span>数据库驱动</span></div></div><div className="settings-note"><strong>权限原则</strong><p>管理员拥有全部权限；其他用户从所属群组合并继承权限。敏感密钥只可覆盖更新，接口不会返回明文。</p></div></div>}
